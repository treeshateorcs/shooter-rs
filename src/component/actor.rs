use crate::{
    model::{ActorActions, ActorActionsExt, BotConfig},
...
impl ActorConfig {
    ...
    pub fn get_image_path(&self, mut suffix: u8) -> String {
        suffix = match self.kind {
            ActorKind::Human => {
                suffix.clamp(1, 2) // player has only 1 and 2
            }
            ActorKind::Zombie => {
                suffix.clamp(0, 1) // zombie has only 0 and 1
            }
        };
